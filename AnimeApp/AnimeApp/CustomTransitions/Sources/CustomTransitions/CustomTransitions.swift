...
public extension AnyTransition {
    static var fly: AnyTransition {
        return AnyTransition.modifier(active: FlyTransition(pct: 0), identity: FlyTransition(pct: 1))
    }
}
...
struct FlyTransition: GeometryEffect {
    ...
    func effectValue(size: CGSize) -> ProjectionTransform {
        ...
        let affineTransform1 = ProjectionTransform(CGAffineTransform(translationX: size.width/2.0,
                                                                     y: size.height / 2.0))
        let affineTransform2 = ProjectionTransform(CGAffineTransform(scaleX: CGFloat(pct * 2),
                                                                     y: CGFloat(pct * 2)))
